...
- **研究目的:** 探究垂直电场下A-A堆叠双层八方石墨烯的能带、磁性及超导电性。
- **主要发现:** 垂直电场能削弱费米面嵌套，抑制自旋密度波序，从而诱导双层八方石墨烯出现非常规$s^{+-}$波超导电性。
...
- **研究目的:** 引入魔术增强Clifford电路，以资源高效的方式实现近似$k$-设计，减少电路深度和魔术门使用。
- **主要发现:** 浅层Clifford电路结合恒定深度魔术门电路，可以生成近似酉和态$k$-设计，显著改善了小$k \geq 4$的电路深度，并减少了魔术门的使用数量。

---

### [Helicons in tilted-Weyl semimetals](http://arxiv.org/abs/2507.02816v1)
- **作者:** Shiv Kumar Ram, Amit Gupta
- **研究目的:** 在倾斜外尔半金属中引入倾斜项，利用半经典玻尔兹曼方法研究螺旋波模式。
- **主要发现:** 倾斜外尔半金属中存在螺旋波模式。
...
- **研究目的:** 利用机器学习预测N、Si、Ge和Sn金刚石空位中心的合成参数，以优化其制备。
- **主要发现:** 通过对大量实验数据进行元分析和机器学习训练，开发出能够准确预测具有特定性质的金刚石材料合成参数的强大工具。
...
- **研究目的:** 利用密度泛函理论研究单斜(In$\_x$Al$\_{1-x}$)$\_2$O$\_3$合金中取代型施主和受主对电子性质的影响，以应用于$\beta$-Ga$\_2$O$\_3$基异质结构。
- **主要发现:** Hf和Zr是Si和其他IV族杂质的良好替代施主，尤其在富氧条件下；Mg、Zn和Cu受主虽然不能促进p型掺杂，但可用于补偿非故意n型掺杂材料。
...
- **研究目的:** 发展量子输运理论，解释飞秒激光脉冲如何在二维反铁磁半导体中通过激子光电流产生的自旋转移矩激发磁振。
- **主要发现:** 飞秒激光脉冲产生的激子光电流通过自旋转移矩激发磁振，其特征可在电荷泵浦和太赫兹发射信号中观察到，为未来实验提供了新的探测手段。
...
- **主要发现:** 光照射能诱导自旋分裂传输特性，抑制热导，并产生有利的自旋热电势和品质因数，其中自旋品质因数在各种光照条件下均优于电荷品质因数。
...
- **研究目的:** 识别控制一维Hubbard模型中自旋和电荷输运不同温度依赖性的机制，并确定其在所有有限温度下的电荷输运性质。
- **主要发现:** 在所有有限温度$T > 0$下，一维Hubbard模型中$\mu = 0$的电荷输运是正常扩散的，这与流体动力学理论和KPZ标度预测相矛盾。
...
- **主要发现:** 硅碳复合材料的初始放电比容量可达6694.21 mAh g$^{-1}$，显著超过硅的理论容量极限，并通过机器学习预测理论上可达7789.55 mAh g$^{-1}$，显示出显著提升锂离子电池能量密度的巨大潜力。
...
- **研究目的:** 开发一种非接触、时空分辨的超快X射线衍射方法，用于测量GaN薄膜中的面内热导率和热边界电导。
- **主要发现:** 该方法成功揭示了GaN薄膜中皱纹缺陷附近的热传输各向异性，显示出局部热导率降低四倍和界面电导下降$25\%$，证明其是表征多层薄膜结构热传输的精确工具。
...
- **研究目的:** 测量高次谐波体声波谐振器(HBAR)中GHz频率模式的激发态布居，并将其用作超冷机械量子传感器来检验新物理。
- **主要发现:** HBAR的基态布居可低至$P\_p = (1.2 \pm 5.5) \times 10^{-5}$，对应有效温度$25.2$ mK，这一结果可用于限制高频引力波、超轻暗物质的动力学混合强度以及薛定谔方程的非线性修正。
...
- **主要发现:** NQS在掺杂二维Hubbard模型上取得了最先进的结果，发现不同的注意力头可以编码不同尺度的关联，并确认了半填充条纹是二维Hubbard模型基态中的稳定结构，与铜氧化物实验观察一致。
...
- **主要发现:** 较高掺杂浓度的掺硼石墨烯能显著增强葡萄糖的拉曼信号，且分子相对于表面的取向对拉曼响应至关重要，表明高浓度掺硼石墨烯是SERS检测葡萄糖的潜在衬底。
...
- **研究目的:** 观测并表征超薄六方氮化硼(h-BN)层中的单光子发射器(SPEs)，并探索其在量子传感中的应用潜力。
- **主要发现:** 在超薄($\simeq 3$ nm) h-BN中观察到由电子束辐照激活的蓝光发射器，通过封装在较厚的h-BN中，其亮度、线宽和光漂白特性得到显著改善，证实了在室温下具有$g^{(2)}(0) < 0.4$的单光子发射，为纳米级空间分辨率的量子传感铺平了道路。

---

### [Pressure-induced band gap energy increase in crystalline lactose](http://arxiv.org/abs/2507.02629v1)
- **作者:** Igor A. Fedorov
- **研究目的:** 研究静水压对结晶乳糖结构性质和电子结构的影响。
- **主要发现:** 晶体乳糖的压缩导致其带隙能量增加，且不同形式结晶乳糖的带隙值与实验数据吻合良好。
...
- **研究目的:** 通过研究一维半填充无自旋$t$-$V$模型的协方差矩阵谱，揭示强关联电荷绝缘体与耦合簇、矩阵乘积和Rokhsar-Kivelson态之间的深层联系。
- **主要发现:** 在强排斥作用下，该模型展现出四位点扰动的后Hartree-Fock图像，并与紧凑的四费米耦合簇态、低秩矩阵乘积态和Rokhsar-Kivelson态存在深刻联系，为理解强关联费米子系统提供了新视角。
...
- **研究目的:** 通过团簇平均场分析，研究具有六个竞争交换相互作用的各向异性海森堡模型中，挫折和量子涨落的相互作用。
- **主要发现:** 外部磁场诱导的量子涨落能抑制局部磁矩并导致磁化平台态，在弱自旋各向异性下，挫折与外场相互作用可产生铁磁和二聚化链共存的奇异量子态。
...
- **研究目的:** 调查Ce基铁磁化合物的硬轴/面有序现象，并探究其驱动机制，特别是晶体电场各向异性和交换相互作用各向异性的作用。
- **主要发现:** 硬轴/面有序在Ce基铁磁体中并不普遍，其主要驱动因素是晶体电场各向异性与交换相互作用各向异性之间的竞争，而非Kondo效应强度，且反铁磁相互作用在稳定硬轴铁磁有序中可能起关键作用。
...
- **研究目的:** 调查Dirac节点线半金属TaNiTe$\_5$中磁阻量子振荡振幅显著增强的现象。
- **主要发现:** 测量装置的机械特性结合磁扭矩中的de Haas-van Alphen振荡，可以在不涉及材料拓扑性质的情况下导致测量电阻的增强。
...
- **研究目的:** 通过密度泛函理论计算和磁扭矩中de Haas-van Alphen振荡的旋转研究，深入了解NbTe$\_4$的电子结构，特别是电子-空穴隧穿现象。
- **主要发现:** NbTe$\_4$在低于$50$K时具有P4/ncc空间群，允许八重简并双Dirac点的存在，并且该材料表现出电子和空穴口袋之间的磁击穿轨道。
...
- **主要发现:** Kagome晶格材料因其独特的几何结构，在磁性、拓扑和强关联效应方面展现出显著优势，为研究自旋、电荷和轨道自由度之间纠缠相互作用提供了有前景的平台。
...
- **研究目的:** 研究非晶拓扑绝缘体纳米线中Aharanov-Bohm振荡和完美透射模式的稳定性。
- **主要发现:** 在低能量和中等非晶化程度下，非晶纳米线的输运仍由完美透射模式主导，该模式受手性对称性或统计时间反演对称性保护；高非晶化程度下，Aharanov-Bohm振荡消失，输运由非量子化共振峰主导，这与拓扑相变相关。
...
- **主要发现:** 辐射损伤与聚苯乙烯基质中自由基的形成有关，且随ZnO:Ga填充量的增加而减少，强调了对复合闪烁体辐射稳定性进行系统和复杂研究的重要性。
...
- **研究目的:** 阐明界面热电效应驱动的摩擦电荷转移的详细动力学，并建立定量的摩擦电荷转移理论。
- **主要发现:** 摩擦电荷在稳态下呈类德尔塔分布，转移的摩擦电荷由热电诱导表面电荷差的一半决定，该理论对理解静电现象和高级能量收集设备具有重要意义。
...
- **研究目的:** 调查4d和5d过渡金属中的磁八极霍尔效应，并探索其微观起源及潜在应用。
- **主要发现:** 磁八极霍尔效应源于轨道织构和自旋-轨道耦合的结合效应，通过计算磁八极霍尔电导率，识别出适合研究磁八极物理的最佳材料，为相关应用提供了有价值的参考。
...
- **研究目的:** 探索在$\mathbb{Z}\_{2}$量子自旋液体(QSL)背景下，单能带扩展$t-J$模型中空穴的动力学，特别是电荷和自旋的相互作用。
- **主要发现:** 发现电子在特定动量下会形成长寿命的自旋子-空穴束缚态，这些束缚态表现为新兴费米子，其费米弧特征与赝能隙相中的观测结果定性一致，为分数化费米液体和高温超导提供了微观机制。
...
- **研究目的:** 报告在铁磁绝缘体钇铁石榴石(YIG)和铂(Pt)双层结构中，在太赫兹频率范围内观测到自旋霍尔磁阻(SMR)效应。
- **主要发现:** 成功在高达1.5 THz的频率下观测到SMR效应，并发现其光谱组成表现出强烈的低通行为，这可以通过横向自旋扭矩和纵向（非相干）自旋流贡献及其特征响应时间的竞争来解释，表明非相干磁振的缓慢耗散是SMR光谱衰减的主要原因。
...
- **研究目的:** 演示在LiNbO$\_3$衬底上高效生成单周期表面声波(SAW)脉冲，以满足薄膜材料调制对高强度SAW的需求。
- **主要发现:** 成功利用啁啾叉指换能器(IDT)在LiNbO$\_3$衬底上生成了0.3 ns FWHM的SAW脉冲，其转换效率比GaAs衬底高约45倍，为自旋电子学和磁子学等领域的高强度SAW应用奠定了基础。
...
- **研究目的:** 为三角晶系PtBi$\_2$中非传统、节点表面超导态构建Ginzburg-Landau理论，以区分不同的超导序参量对称性并预测磁场效应。
- **主要发现:** 构建了包含三种超导序参量（对应$C\_{3v}$点群的$A\_1$和$A\_2$不可约表示）的Ginzburg-Landau泛函，并利用其确定了磁场对$A\_1$和$A\_2$配对的节点结构的影响，预测了场致对密度波，为实验区分这些对称性提供了明确指导。
...
- **研究目的:** 报告开发一种结合超快、高重复率、偏振可调谐单色极紫外(XUV)光束线和下一代动量显微镜的仪器，以实现量子材料的时间和角度分辨光电子能谱测量。
- **主要发现:** 成功开发了该仪器，实现了44 meV的能量分辨率和144 fs的时间分辨率，并展示了其在光激发二维材料中进行线偏振、傅里叶和圆偏振光电子角分布二向色性测量的能力，为研究量子材料的轨道和量子几何特性提供了新途径。
...
- **研究目的:** 开发一种高通量NEB计算框架，结合微调的通用机器学习原子间势(uMLIPs)，以加速锂离子迁移势垒的预测，从而高效发现快离子导体。
- **主要发现:** 成功开发了集成微调CHGNet模型的NEB/MD高通量计算框架，实现了计算速度和准确性的平衡，并通过高通量筛选识别出正交Pnma群结构（如LiMgPO$\_4$、LiTiPO$\_5$）作为有前景的快离子导体骨架，并预测其掺杂变体具有低活化能和高离子电导率。

---

### [Nodal-line semimetals and their variance](http://arxiv.org/abs/2507.02329v1)
- **作者:** Po-Yao Chang
- **研究目的:** 综述拓扑节点线半金属(NLSMs)的各种类型、特征电磁响应以及强关联效应，以增进对其独特物理性质的理解。
- **主要发现:** 总结了NLSMs根据节点结构和保护对称性的分类，强调了其独特的鼓面表面态和电磁响应（如Landau能级谱和光学电导率），并指出强关联效应可以修改其半金属相，导致磁性和超导性交织的新型量子相。
...
- **研究目的:** 检验一维(1D)调制对称性保护拓扑(MSPT)相中的非可逆对称性(NIS)，特别是偶极和指数电荷对称性保护的MSPT。
- **主要发现:** 明确构建了偶极和指数电荷对称性保护的MSPT中的非可逆Kramers-Wannier和Kennedy-Tasaki变换，揭示了具有自发对称性破缺(SSB)的对偶模型，并建立了拓扑全息对应，将1D MSPT相与2D体理论联系起来。
...
- **研究目的:** 通过界面工程，提高金刚石中浅层氮空位(NV)中心的量子相干时间，以克服其受噪声影响的短相干时间问题。
- **主要发现:** 通过氧终端化和石墨烯修补的界面工程，将浅层NV中心的相干时间延长至1毫秒以上，接近$T\_1$极限，并通过拉曼光谱和密度泛函理论揭示了石墨烯电荷转移通过配对表面电子来减少自旋噪声的机制，显著提升了量子传感器的性能。
...
- **研究目的:** 利用对称性论证，揭示哈伯德模型中电子-电子相互作用的矩阵元在布里渊区高对称点附近波矢组成的态中达到极值，并探讨其对有限动量配对的促进作用。
- **主要发现:** 证明了电子-电子相互作用的矩阵元在布里渊区高对称点附近的波矢态中达到极值，这种依赖性源于波函数波矢相关的轨道组成，为寻找新型高温超导体提供了基于对称性的指导。
...
- **研究目的:** 探索三元碲化锆Zr$\_6$RuTe$\_2$的超导性质。
- **主要发现:** 发现Zr$\_6$RuTe$\_2$在$T\_c = 1.1$ K以下表现出体超导性，其超导转变温度高于其他Zr$\_6$MTe$\_2$化合物，表明Ru元素有利于该类材料的超导性。
...
- **研究目的:** 建立一种通用方法来研究耦合到一维非厄米单能带浴的单个量子发射体的散射态。
- **主要发现:** 建立了研究一维非厄米浴中单个量子发射体散射态的通用方法，发现在热力学极限下，散射态通常不再是平面波的线性叠加，而是表现出与晶格尺寸成比例的有限局域长度，并讨论了Lippmann-Schwinger方程失效的情况。
...
- **研究目的:** 通过大规模微磁模拟，全面绘制磁性纳米花在不同尺寸范围内的磁化行为，以理解其微观结构与宏观磁响应之间的关系，并优化磁热疗性能。
- **主要发现:** 绘制了磁性纳米花从10到400 nm尺寸范围的矫顽力-尺寸图，发现其在涡旋态下存在两种不同的反转模式，并揭示了内部自旋无序在磁热疗加热中的关键作用，通过调整晶粒尺寸可以定制矫顽力“最佳点”，从而实现下一代高效纳米加热器。
...
- **研究目的:** 利用输运、热力学和散射实验研究Ba$\_{1-x}$Sr$\_x$Al$\_4$和Ba$\_{1-y}$Eu$\_y$Al$\_4$固溶体系列中非公度电荷有序态的演化，并揭示其起源。
- **主要发现:** 发现SrAl$\_4$中存在非公度电荷密度波(CDW)态，并随着Ba替代而演化，CDW相的抑制与声子不稳定性的抑制精确匹配，证实了该系统中电荷有序的电子-声子起源，并揭示了CDW转变温度和有序波矢之间的普适关联。
...
- **研究目的:** 系统地制造和表征硅悬挂键(DBs)在H-Si(100)表面上的六种平面原子线配置，以理解其几何结构如何影响电子态，从而为原子尺度电路设计提供指导。
- **主要发现:** 通过低温扫描隧道显微镜和谱学(STM/STS)以及密度泛函理论(DFT)计算，揭示了不同原子线几何结构对局域态密度(LDOS)和轨道杂化的影响，发现二聚体和更宽的原子线展现出多个离散的带隙中电子态和更高的传输系数，为原子尺度电路的信号传输和量子点应用提供了新途径。
...
- **研究目的:** 综述太赫兹二维相干光谱(THz-2DCS)技术在探测、可视化和控制非平衡量子材料方面的进展，特别是其在揭示多体动力学和多阶关联方面的能力。
- **主要发现:** THz-2DCS通过在时间和频率维度上映射复杂激发，实现了驱动量子物质的相干层析成像，揭示了隐藏的激发路径、测量了高阶非线性响应函数、解缠了各种量子路径，并在超快时间尺度和太赫兹频率下捕捉了集体模式，为非平衡超导、非线性磁子学和动态拓扑相的研究提供了强大工具。
...
- **研究目的:** 研究石墨烯量子霍尔(QH)态与超导体界面处纳米结构中的安德烈夫转换，特别关注最低朗道能级。
- **主要发现:** 发现QH态下的安德烈夫转换对界面透明度比零磁场情况更具鲁棒性，并且对于部分透明的界面，混合电子-空穴界面模式不再谷简并，且即使在无序的情况下，角部也可能发生谷间散射，导致两种混合模式之间的干涉。
...
- **研究目的:** 调查一维无序亚波长谐振器系统中，由虚规范势引起的非互易阻尼与无序之间，在局域化机制上的竞争。
- **主要发现:** 阐明了虚规范势导致的边缘局域化与无序导致的体局域化之间的竞争，并通过Lyapunov指数表明无序在小虚规范势下可以起到绝缘作用，阻止边缘局域化。
...
- **研究目的:** 数值研究静置胶体液滴蒸发过程中胶体颗粒的传输及其在不利条件下的沉积模式。
- **主要发现:** 结果表明，在颗粒-基底相互作用存在的情况下，向外的径向内部流动是导致“咖啡环效应”形成环状沉积模式的主要原因，而热马兰戈尼内部流动则能减弱这种效应，形成均匀沉积并带有深色边缘。
...
- **研究目的:** 探索手性钙钛矿在非常规自旋电子学中的应用潜力，特别是其诱导自旋极化载流子、电流和激发的机制。
- **主要发现:** 通过第一性原理研究和轨道对称性分析，阐明了二维PbBr$\_4$基手性钙钛矿的电子结构和手性起源，并提出Edelstein效应可能是手性诱导自旋选择性(CISS)的机制之一，从而将手性钙钛矿确立为非常规自旋电子学的多功能材料平台。
...
- **研究目的:** 重新审视单层1H-NbSe$\_2$及其相关化合物的电子结构和能带拓扑，并构建精确的Wannier函数模型，以理解其量子几何对关联相的影响。
- **主要发现:** 构建了6带、3带和单带模型，发现所有模型在费米能级附近都存在远离原子位置的受阻原子孤立能带，且最简单的单带模型中次近邻跳跃项远大于近邻跳跃项，为未来研究该材料家族中量子几何对关联相的影响奠定了基础。
...
- **研究目的:** 证明手性$\mathbb{Z}\_p$环面码模型在受到微扰时，会产生额外的、新兴的拓扑相，即“Hall-on-Toric”状态，这些状态类似于分数量子霍尔态。
- **主要发现:** 通过iDMRG模拟验证了“Hall-on-Toric”相的存在，这些分层态具有分数化的$\mathbb{Z}\_p$电荷和增加的拓扑基态简并度，并且即使在没有$U(1)$对称性的情况下也保持鲁棒性，揭示了环面码中磁性和电激发更深层次的解释。
...
- **研究目的:** 将对称性拓扑场论(SymTFT)框架扩展到内部对称性之外，通过包含几何数据来编码时空对称性，以研究时空和内部对称性相互作用导致的对称性富集拓扑(SET)序。
- **主要发现:** 成功将SymTFT框架扩展以包含时空对称性，并以1+1D对称性为例，阐明了Lieb-Schultz-Mattis(LSM)反常与调制对称性之间的关系，分类了调制对称性保护拓扑(SPT)相，并识别了混合反常，为理解更复杂的对称性保护拓扑相提供了统一框架。
...
- **研究目的:** 探索二维非厄米Su-Schrieffer-Heeger (SSH)模型中的“拓扑实数开关”，旨在实现可控地诱导实数谱，并为体-边界选择性激光提供基础。
- **主要发现:** 在二维非厄米SSH模型中引入并展示了一个“实数开关”，该开关能够根据施加的边界条件可控地诱导实数谱，并且伴随着复数能隙中的拓扑相变，为非厄米系统中增益和激光行为的体-边界选择性控制机制奠定了基石。
