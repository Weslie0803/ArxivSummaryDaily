...
- **研究目的:** 探究$d$-波交错磁体中反常平面输运现象，特别是反常平面霍尔效应和平面热霍尔效应。
- **主要发现:** 这些系统在低温下表现出近似半整数量子化的反常平面霍尔效应和平面热霍尔效应，并在一定磁场范围内持续存在，其角依赖性呈$\cos2\phi$形式。
...
- **研究目的:** 运用可精确求解的多融合Hu-Geer-Wu弦网模型，揭示非阿贝尔任意子在对称性富集拓扑(SET)相中其内部规范空间如何随对称性变换。
- **主要发现:** 发现了一种普适机制，即全局对称性碎裂(GSF)，其中对称性不变的任意子表现出内部希尔伯特空间分解为由分数对称电荷标记的本征子空间，从而实现了真正的非线性对称性表示。
...
- **研究目的:** 研究简单立方晶格上具有次近邻跳跃的稀释半填充Hubbard模型中的磁输运性质，以探索稀释反铁磁系统中的金属性。
- **主要发现:** 半经典蒙特卡洛计算揭示了稀释关联材料中存在反铁磁金属态，其中位点稀释在不显著改变底层长程反铁磁有序的情况下诱导了金属性。
...
- **研究目的:** 首次从第一性原理角度深入研究半赫斯勒合金$\text{CrMnSb}$，以理解其偏离传统Slater-Pauling半导体行为的原因。
- **主要发现:** 尽管$\text{CrMnSb}$满足18个价电子的Slater-Pauling规则，但它表现出半金属、完全补偿的亚铁磁基态，这源于局域亚晶格磁矩以及$\text{Cr}$和$\text{Mn}$亚晶格之间的反平行排列。
...
- **研究目的:** 研究稀土前驱体（镧）的改性如何通过低温球磨提高氢反应活性和吸氢量，从而在较低压力下形成金属氢化物。
- **主要发现:** 低温球磨系统地增强了镧对氢的吸收，促进了$\text{LaH}\_{x}$ ($x=2-3$)相的形成，并在高达60 GPa的压力下实现了超价$\text{LaH}\_{4}$相的形成，这与前驱体低温球磨后$\text{Fm3-m}$立方结构菱形畸变的抑制有关。
...
- **研究目的:** 首次展示基于$\text{La}$掺杂$\text{CaSnO}\_{3}$的高性能金属氧化物半导体场效应晶体管（MOSFET），以满足高电压和高功率电子应用的需求。
- **主要发现:** $\text{CaSnO}\_{3}$ MOSFET器件表现出超过$10^{8}$的开关比、$8.4 \text{ cm}^{2} \text{V}^{-1} \text{s}^{-1}$的场效应迁移率、$30 \text{ mA mm}^{-1}$的通态电流，以及$1660 \text{ V}$的关态击穿电压和$\sim 8.3 \text{ MV cm}^{-1}$的击穿场强，达到超宽带隙半导体中的最高水平。
...
- **研究目的:** 探讨定义在Chern带上的有效相互作用的Chern数如何与分数Chern绝缘体（FCIs）的稳定性相关联，通过统一的矩阵形式化有效相互作用和双粒子问题。
- **主要发现:** 数值计算发现，主导的双粒子能带携带单位Chern数与填充分数$\nu = 1/3$处稳健FCI的实现之间存在显著关联，这表明相互作用本身固有的拓扑性质可以阻碍平庸基态的形成。
...
- **研究目的:** 利用密度泛函理论计算，研究$\text{NO}$退火引入的氮化层对$4\text{H-SiC}(0001)/\text{SiO}\_{2}$界面的电子结构和载流子散射特性的影响。
- **主要发现:** 氮化层能够屏蔽$\text{SiO}\_{2}$中$\text{O}$原子不利的库仑相互作用，但由于氮原子面密度较低，流经氮化层下方的电子会受到显著的势能波动散射。
...
- **研究目的:** 通过固态锂插层、电阻测量和隧穿光谱技术，探测锂插层$1\text{T-TiSe}\_{2}$的超导能隙，并研究其行为。
- **主要发现:** 超导能隙闭合温度远超$\text{BCS}$理论预测的临界温度($T\_c$)，表明存在显著的超导涨落；对称能隙在反常金属态中依然存在；且超导能隙表现出高次谐波的磁振荡，证实了内在超导网络的规则结构。
...
- **研究目的:** 揭示具有强几何或电各向异性纳米结构之间范德华(vdW)色散相互作用的反常标度律。
- **主要发现:** 发现各向异性纳米结构之间存在反常的$d^{-10}$标度律，其内在驱动力是受对称性限制的等离激元相互作用，并在有限各向异性比下因等离激元模式竞争而发生从$d^{-10}$到$d^{-6}$的标度律转变。
...
- **研究目的:** 利用密度泛函理论计算，研究单层镍酸盐$\text{La}\_{2}\text{NiO}\_{4}$在压力和掺杂作用下的结构和电子性质。
- **主要发现:** 四方$I4/mmm$结构不仅在约8 GPa的低压下可以稳定，通过$\text{Ba}$部分取代$\text{La}$在常压下也能稳定，并且$\text{Ba}$取代和压力的共同作用导致电子结构向超导双层镍酸盐的$d^{7.5}$构型发生质变，增强了不同磁态之间的竞争。
...
- **研究目的:** 证明通过大范围、精细控制的各向异性应变，可以有效调节$\kappa\text{-(ET)}\_{2}\text{Cu}\_{2}\text{(CN)}\_{3}$这种各向异性三角晶格量子磁体中几何阻挫的程度。
- **主要发现:** 弹性热效应的热力学测量绘制出了一个温度-应变相图，捕捉了各向同性晶格的基态和阻挫较小的母态，为三角晶格Hubbard模型随阻挫变化的计算提供了新的基准。
...
- **研究目的:** 对超导环形谐振器进行理论和实验研究，作为其在超导电子学和量子技术中应用的第一步。
- **主要发现:** 建立了描述微波行为关键特征的超导环形谐振器模型和信号流图，并实验验证了频率分裂和模式旋转在耦合简并模式的环形系统中普遍存在，并在高品质因数超导环形谐振器中得到清晰分辨。
...
- **研究目的:** 设计一种由电中性石墨烯纳米带在外部磁场下形成的人工二维量子材料，以控制和塑造量子极化激元，使其获得双曲色散。
- **主要发现:** 在这种超表面中，量子极化激元获得了双曲色散，其等频曲线的拓扑结构可以通过调节外部磁场强度从闭合变为开放，并在拓扑转变时观察到所有极化激元平面波沿相同方向传播的通道化现象。
...
- **研究目的:** 开发一种微波技术，用于在高达125 T的半破坏性脉冲磁场和低至1.5 K的低温下进行高灵敏度的两点输运测量。
- **主要发现:** 新的实验装置成功地首次观测到$\text{WTe}\_{2}$在超过100 T磁场下的Shubnikov-de-Haas振荡，证明了其在极端条件下进行精确测量的能力。
...
- **研究目的:** 研究钴掺杂对斯格明子宿主材料$\text{Cu}\_{2}\text{OSeO}\_{3}$原子和磁性结构的影响。
- **主要发现:** 钴掺杂导致晶胞膨胀和材料磁有序的显著变化，包括临界磁场增加、螺旋磁相临界温度降低，以及斯格明子口袋在更宽温度范围内的稳定。
...
- **研究目的:** 提出对称性分析的单晶共振X射线布拉格衍射磁结构因子，利用$\text{Eu}$原子共振来解析$\text{EuPdSn}\_{2}$的磁结构。
- **主要发现:** 分析表明共振X射线布拉格衍射技术具有显著潜力，并证实由于$\text{Eu}$非中心Wyckoff位置，轴向和极性$\text{Eu}$多极矩是磁中子和共振X射线布拉格衍射图样中必不可少的组成部分。

---

### [Two-Dimensional Materials-Based Josephson Junctions](http://arxiv.org/abs/2506.23737v1)
- **作者:** Hamidreza Simchi
- **研究目的:** 研究基于二维单层$\text{MoS}\_{2}$的约瑟夫森结的量子输运性质，该结由中间半导体薄片和半无限拓扑及非拓扑超导引线组成。
- **主要发现:** 当化学势的绝对值远小于超导配对势时，拓扑引线中形成Chern数为二的马约拉纳零模；而对于普通超导引线，约瑟夫森电流呈正弦行为（由于安德烈夫束缚态），但对于拓扑超导引线，约瑟夫森电流为零，从而可将其视为一个两态开关。
...
- **研究目的:** 对基于晶格的反铁磁斯格明子在自旋转移矩驱动下的动力学进行数值研究，并详细进行二维经典XY模型的蒙特卡洛研究，以量化空间各向异性和Dzyaloshinskii-Moriya (DM)耦合如何重塑其热响应。
- **主要发现:** 研究表明，调节$J\_{y}/J\_{x}$比值系统地改变了比热异常，证实了从准1D涨落到2D Kosterlitz-Thouless转变的交叉，而引入DM相互作用则通过提高峰值幅度和产生低温平台进一步丰富了相图。

---

### [Photonic obstructed atomic insulator](http://arxiv.org/abs/2506.23713v1)
- **作者:** Hongyu Chen
- **研究目的:** 探究二维磁性光子受阻原子绝缘体中角态的出现及其与Wyckoff位置的关联。
- **主要发现:** 当超晶格暴露$2b$ Wyckoff位置时，角态出现，且随着暴露的$2b$ Wyckoff位置数量减少而减弱，当$2b$ Wyckoff位置不再位于边缘时消失，表明原子的实空间分布决定了受阻原子绝缘体的高阶拓扑绝缘体特征。
...
- **研究目的:** 开发并验证一种依赖于电子温度的铜相互作用势，并将其与硅进行比较，用于激光烧蚀的原子模拟，考虑强电子激发效应。
- **主要发现:** 成功开发了一种考虑电子激发至$1.2 \text{ eV}$的铜相互作用势，并证明其在自由能曲线、弹性常数、声子谱以及熔化温度随激发程度的变化方面与所有这些性质保持一致，且与硅的行为具有可比性。
...
- **研究目的:** 利用单晶同步辐射X射线衍射实验，重新研究尖晶石型化合物$\text{CuIr}\_{2}\text{S}\_{4}$在低温下的晶体结构，以澄清其电荷有序二聚体模式。
- **主要发现:** 重新分析结果证实了$\text{Ir}$二聚体的形成伴随着电荷有序，但揭示了与先前报道的粉末衍射研究不同的电荷有序模式和晶胞内二聚体排列。
...
- **研究目的:** 识别一种同时具有高载流子迁移率和强电子关联效应的范德华(vdW)材料，并重点研究$\text{CeTe}\_{3}$。
- **主要发现:** $\text{CeTe}\_{3}$在低温下表现出显著的有效质量增强，这源于反铁磁有序和窄带关联效应，但量子迁移率却出人意料地增加到$\sim 2403 \text{ cm}^{2}/\text{Vs}$（在纳米薄片中达到$\sim 3158 \text{ cm}^{2}/\text{Vs}$），可能得益于拓扑保护。
...
- **研究目的:** 在赝势平面波代码中，从第一性原理实现Bloch电子的量子几何性质（Berry曲率、量子度规、轨道磁矩和有效质量）的精确计算。
- **主要发现:** 该方法基于求解波函数对波矢$k$的一阶导数的Sternheimer方程，能够高精度地计算有效质量，并成功应用于带隙石墨烯和三角碲的量子几何量计算，证实了其与手性相关的符号翻转。
...
- **研究目的:** 采用具有离散能级的替代$\text{BCS}$模型，研究一个由两个量子点($\text{QD1}$和$\text{QD2}$)组成的混合系统中的量子相变，其中$\text{QD1}$通过隧穿耦合到两个超导引线。
- **主要发现:** 精确对角化揭示了丰富的相图，其中包含由量子点相互作用介导的多个可控相变，包括通过调节$\text{QD2}$相互作用、$\text{QD1}$相互作用以及点间耦合引发的相变，并可在特定参数条件下通过平行磁场驱动可逆的铁磁-反铁磁相变。
...
- **研究目的:** 研究块状Ni-Mn-Ga奥氏体中磁畴及其在磁场作用下的动态行为。
- **主要发现:** 利用先进的克尔显微镜首次在块状Ni-Mn-Ga奥氏体中观察到磁畴，并发现其表面矫顽力远高于体相，且磁畴仅沿一个立方轴方向排列。
...
- **研究目的:** 探究高度无序分形MgO/MgB$\_2$纳米复合材料中全局相位相干性的建立机制，并分析界面、形貌和缺陷的作用。
- **主要发现:** 该纳米复合材料中存在各向同性分布的复杂微结构和原子级清洁的界面，且氧空位通道有助于长程载流子传输，从而促进了超导相位相干性的建立。
...
- **研究目的:** 利用第一性原理计算和对称性分析，研究手性晶体SiTc的拓扑电子和声子性质。
- **主要发现:** SiTc晶体同时存在多重简并和拓扑节点，并展现出拓扑非平凡的电子和声子模式，包括外尔点、费米弧和本征自旋霍尔电导率。
...
- **研究目的:** 结合实验和动力学蒙特卡罗模拟，揭示限制有机热电材料性能的关键因素，尤其是在中间掺杂区域。
- **主要发现:** 有机热电材料的最佳热电功率因数($PF\_{max}$)始终出现在常规变程跳跃(VRH)与费米能级处形成软库仑间隙的Efros-Shklovskii变程跳跃(ES-VRH)之间的转变点。
...
- **研究目的:** 利用第一性原理计算研究垂直ZrS$\_2$/HfS$\_2$范德华异质结的电子、光学和激子性质，特别关注激子效应及其温度依赖性。
- **主要发现:** ZrS$\_2$/HfS$\_2$异质结表现出间接带隙和I型带对齐，其光学带隙和激子结合能随温度升高而减小，表明电子-声子相互作用对光电响应有重要影响。
...
- **研究目的:** 寻找一种有效的方法，将矩阵乘积态(MPSs)近似转换为受限玻尔兹曼机波函数，以用于量子多体基态计算。
- **主要发现:** 通过对MPSs进行规范多项式分解(CP分解)，可以高效生成初始的神经网络量子态，并系统地缩短初始态与基态之间的距离。
...
- **研究目的:** 探索如何在光子晶体中模拟交错磁性(altermagnetism)，并利用其独特的磁性对称性工程电磁波。
- **主要发现:** 工程化的光子晶体可以承载自旋空间群对称性，从而产生自旋分裂能带，并实现无需几何动力学自旋-轨道相互作用的圆偏振光隔离。
...
- **研究目的:** 利用量子电动力学密度泛函理论(QEDFT)计算，研究光子场在光子结构中如何通过偏振介导的对称性破缺机制，在块状HgTe中诱导新兴的拓扑相。
- **主要发现:** 强光-物质耦合可以重构HgTe的电子和离子结构，根据样品在光子结构中的相对取向和耦合强度，将其驱动进入外尔、节点线或拓扑绝缘体相。
...
- **研究目的:** 探索通过氧空位有序化来控制关联氧化物VO$\_2$中的绝缘体-金属相变(IMT)，并揭示其诱导的拓扑相变路径。
- **主要发现:** 揭示了VO$\_2$-VO$\_{2-x}$-V$\_2$O$\_3$连续的氧空位驱动拓扑相变路径，该路径具有诱人的晶面依赖各向异性，并能实现IMT性质在扩展温度范围内的可调控性。
...
- **研究目的:** 研究螺旋位错和外部磁场同时作用下，二维量子环中带电无自旋粒子的光学和量子力学性质。
- **主要发现:** 螺旋位错通过引入与能量相关的有效势，不仅能改变共振峰的位置，还能增强或抑制特定的光学跃迁，从而为拓扑量子器件中的光-物质相互作用提供了几何调控的可能性。
...
- **研究目的:** 研究旋转对二维量子环中费米能、磁化强度和持久电流的影响。
- **主要发现:** 旋转对量子环的费米能、磁化强度和持久电流有显著影响，表明旋转可以作为一种无需额外场或几何修改的控制参数，用于开发新型介观器件。
...
- **研究目的:** 发现并理解一维反铁磁海森堡模型中连续对称性的自发破缺现象。
- **主要发现:** 发现了一维反铁磁海森堡模型中存在连续对称性的自发破缺，该模型并非无挫折且其序参量不与哈密顿量对易，但其长程有序对对称性保持的微扰具有鲁棒性。
...
- **研究目的:** 开发一种快速、低剂量、亚20皮米精度的电子束定位技术，以实现对单原子的精确探测和控制。
- **主要发现:** 研发了“原子锁定”(ALO)技术，实现了对特定原子列的亚20皮米精度电子束定位，从而能够定量测量单原子事件并观察原子双稳态等动力学行为。
...
- **研究目的:** 构建一个广义的电阻-电容并联结(RCSJ)模型，为约瑟夫森二极管效应提供一个统一的唯象学框架。
- **主要发现:** 提出了理想二极管效应（需要同时破缺$\mathcal{I}$和$\mathcal{T}$对称性）和伪二极管效应（$\|I\_{c+}\|=\|I\_{c-}\|$但$\|I\_{r+}\|\ne\|I\_{r-}\|$）的分类，并展示了Shapiro台阶可用于区分这三类二极管效应。
...
- **作者:** Nikita Medvedev
- **研究目的:** 评估CrMnFeCoNi高熵合金在升高电子温度下的热力学性质，包括电子热容、热导率和电子-声子耦合强度。
- **主要发现:** 在电子温度约24,000 K时，CrMnFeCoNi合金即使原子温度未升高，也会因电子激发引起的原子间势能改变而发生非热熔化。
...
- **研究目的:** 提出一种通过利用电场激发低能量极性光学声子模式来调控非谐钙钛矿型半导体光电性质的新策略。
- **主要发现:** 极性声子畸变可以在环境条件下引起显著的带隙调制，例如Ag$\_3$SBr中带隙减小高达70%，而BaTiO$\_3$中带隙增加近23%，这为设计可调谐光电器件提供了新途径。
...
- **研究目的:** 研究共形场论中无质量重整化群(RG)流连接的融合环之间的环同态，并提出一种广义对称性的量子哈密顿量形式。
- **主要发现:** 融合环的理想在任意子凝聚规则中扮演着基础角色，其理想分解为具有非可逆对称性的有隙相及其对称性破缺模式提供了直接而强大的约束。
...
- **研究目的:** 证明超导层在两个铁磁绝缘体之间可以驱动反铁磁交换耦合，并实现由该耦合引起的双稳态和绝对开关效应。
- **主要发现:** 利用夹在两个铁磁绝缘体GdN层之间的超导V薄膜，成功实现了超导交换耦合驱动的零电阻态和有限电阻态之间的双稳态和绝对开关，为低温非易失性存储器提供了应用潜力。
...
- **研究目的:** 利用分子动力学(MD)模拟，深入研究C-S-H凝胶孔隙中离子和水分子的扩散行为，包括其空间分布、能垒和结构描述符。
- **主要发现:** 揭示了纳米受限环境中扩散系数在固液界面附近受到强烈抑制，并提出了总配位强度(TCS)作为预测界面附近分子迁移率的结构描述符。
...
- **研究目的:** 引入结合密度泛函理论(DFT)和非平衡格林函数(NEGF)的框架，计算纳米电容器的电容，并直接提取亚纳米介质在偏压下的介电响应。
- **主要发现:** 提出了一种稳健的电荷分离协议，能够得到独特的电容导出极化率和介电常数，明确表明限制不会改变冰的固有电子响应或其对质子有序性的不敏感性。
...
- **研究目的:** 理论研究由Néel自旋-轨道转矩(NSOTs)引起的共线反铁磁体的共振和高次谐波磁响应。
- **主要发现:** 发现NSOTs能够将线性和非线性磁动力学放大数个数量级，从而在亚太赫兹频率范围内实现高效的频率转换，揭示了NSOTs在非线性动力学中先前被忽视的作用。
...
- **研究目的:** 提出一种利用面积探测器在掠入射X射线散射实验中实现样品表面与X射线束精确对准的简便方法。
- **主要发现:** 该方法通过观察样品的反射和透射束，能够确定样品-探测器距离和最佳马达位置，从而实现散射X射线强度的增强。
...
- **研究目的:** 回顾并讨论Ising自旋-1/2 XXZ链中超出常规自旋子范畴的量子问题。
- **主要发现:** 强调了无自旋子表示的Bethe自旋串对动力学性质的贡献，以及在任意有限温度下弹道自旋输运相关的自旋刚度，这些都超出了传统的自旋子描述。
...
- **研究目的:** 综述交错磁性(altermagnetism)的研究背景，并重点阐述其对称性、显微镜和光谱学特征。
- **主要发现:** 交错磁性是一种独特的共线补偿磁有序相，具有d、g或i波各向异性以及在位置和动量空间中交替的电子结构自旋极化，其独特的对称性、微观结构和电子光谱特征使其区别于传统磁性材料。
...
- **研究目的:** 利用分子模拟探究活性诱导的超螺旋如何通过捕获穿线缠结来减缓质粒溶液的弛豫。
- **主要发现:** 活性诱导的超螺旋不仅将聚合物的构象拓扑从开放变为分支，还将穿线环锁定在超分子簇中，使其弛豫非常缓慢，从而为调控大分子系统的动态行为提供了新途径。
...
- **研究目的:** 研究HfNbTaTiZr难熔高熵合金在单轴拉伸下的尺寸依赖性力学行为，重点关注随机固溶（RSS）和化学短程有序（CSRO）的影响。
- **主要发现:** 结果表明，随机固溶（RSS）增强了强度，而化学短程有序（CSRO）降低了流变应力水平但提高了应变硬化和抗失效性，并观察到Hall-Petch强化向反Hall-Petch软化的转变，其中CSRO抑制了这种转变。
...
- **研究目的:** 综述位错在陶瓷材料中对机械性能和关键功能特性（如输运、铁电性、导热性和超导性）的影响。
- **主要发现:** 该综述旨在为陶瓷材料中的位错工程提供新见解，展示了位错如何影响陶瓷的机械和功能特性，包括输运、铁电性、导热性和超导性。
...
- **研究目的:** 演示一种基于层状高温超导体的鲁棒片上太赫兹发射器，该发射器采用椭圆形微腔设计，并能实现超过11年的持续相干发射。
- **主要发现:** 该研究成功展示了一种基于层状高温超导体的片上太赫兹发射器，其具有超过11年的持续相干发射寿命，为可扩展、可调谐的固态相干太赫兹激光器平台开辟了道路。
...
- **研究目的:** 报告在室温下从大面积可转移垂直排列的手性准二维钙钛矿超晶格的边缘态中观察到的手性圆偏振超荧光。
- **主要发现:** 该研究首次在室温下从钙钛矿超晶格中观察到手性圆偏振超荧光，并揭示了手性在建立大规模量子相干性中的关键作用，为手性控制的量子自旋光学应用开辟了新途径。
...
- **研究目的:** 提出一种剪切应变策略，以将d波交变磁体显著调制为非交变补偿磁体，并探索其在磁性隧道结（MTJ）器件设计中的应用潜力。
- **主要发现:** 该研究提出了一种剪切应变策略，可将d波交变磁体显著调制为非交变补偿磁体，并发现这种非交变电子结构不仅增强了本征$RuO\_2$自旋分裂路径中的隧穿磁阻（TMR），还在自旋简并路径中实现了显著的TMR。
...
- **研究目的:** 扩展深度学习目标检测算法，以识别贝氏体显微组织中显著影响其力学性能的马氏体-奥氏体（MA）岛。
- **主要发现:** 该研究开发了一种基于深度学习的单次检测算法，成功识别了贝氏体显微组织中的马氏体-奥氏体（MA）岛，实现了贝氏体钢中第二相的可扩展自动化检测。
...
- **研究目的:** 通过引入条带广义布里渊区（SGBZ）框架，建立二维几何依赖非厄米趋肤效应的非布洛赫能带理论。
- **主要发现:** 该研究通过引入条带广义布里渊区（SGBZ）框架，建立了二维几何依赖非厄米趋肤效应的非布洛赫能带理论，为探索二维晶格系统中的非厄米效应和设计具有定制边界行为的新型非厄米材料及器件提供了通用路线图。
...
- **研究目的:** 研究具有面内极化的二维铁电体中纵向非线性输运受极化和磁场控制的特性。
- **主要发现:** 该研究揭示了二维铁电体中纵向非线性输运的各向异性特征及其受极化和磁场控制的机制，为设计极化控制的整流器件提供了指导。
...
- **研究目的:** 分析由双量子自旋霍尔绝缘体和电介质周期性堆叠层状结构表面弱耦合的双螺旋边缘态，以探索其中可调谐的竞争性电子序。
- **主要发现:** 该研究通过重整化群分析，揭示了双量子自旋霍尔超晶格中可调谐的竞争性二维$\pi$-超导（SC）和$\pi$-自旋密度波（SDW）序，并提出了一个探索这些序的材料平台。
...
- **研究目的:** 研究通过固态反应合成的多晶$CeCr\_xFe\_{1-x}O\_3$（$x = 0, 0.33, 0.67, 1$）的磁性行为。
- **主要发现:** 该研究发现，在多晶$CeCr\_xFe\_{1-x}O\_3$中，掺杂$Cr^{3+}$会导致异常的双重自旋重取向转变，即在约230 K和100 K附近分别发生$\Gamma\_4 \rightarrow \Gamma\_1$和$\Gamma\_4 \rightarrow \Gamma\_2$转变。
...
- **研究目的:** 揭示插层过渡金属二硫化物$V\_{1/3}NbS\_2$能够结晶成两种由面外磁性插层剂周期性区分的有序超晶格。
- **主要发现:** 该研究发现$V\_{1/3}NbS\_2$可以结晶成两种非常规有序超晶格，其中一种是半金属非共线反铁磁体，这为调控相关材料中丰富的磁性和电子行为提供了新途径。
...
- **研究目的:** 提出一种新颖的方法，通过光学吸收和阻抗谱测量，准确确定卤化物钙钛矿中的激子结合能和约化有效质量，并充分考虑载流子与纵向光学声子的相互作用引起的极化效应。
- **主要发现:** 该研究提出了一种新颖的方法，结合光学吸收和阻抗谱测量，并考虑载流子与纵向光学声子的相互作用引起的极化效应，以准确确定金属三卤化物钙钛矿中的激子结合能和约化有效质量。

---

### [Alter-Piezoresponse in Two-Dimensional Lieb-Lattice Altermagnets](http://arxiv.org/abs/2506.22663v1)
- **作者:** Xilong Xu, Li Yang
- **研究目的:** 预测在Lieb晶格交变磁体（特别是过渡金属硫族化合物$M\_2WS\_4$，$M = Mn, Fe, Co$）中新颖的实空间交变压磁和压电响应。
- **主要发现:** 该研究预测了二维Lieb晶格交变磁体中新颖的实空间交变压磁和压电响应，并指出这种交变压电响应为独立精确控制电磁特性提供了前所未有的机会，有望应用于高保真多功能存储和传感器。
...
- **研究目的:** 通过反射率对比光谱观察双层$WSe\_2$在施加电场下的层间激子A1s^I、A2s^I和B1s^I，并揭示其增亮机制。
- **主要发现:** 该研究通过反射率对比光谱观察到双层$WSe\_2$中电场驱动的层间空穴转移可使层间激子增亮，并确定层间空穴转移是双层过渡金属二硫化物中层间激子增亮的一种普遍且稳健的机制。
...
- **研究目的:** 开发一种超高温真空探针台（UHT-VPS），用于在极端条件下对材料进行可靠的电学和热学测量。
- **主要发现:** 该研究开发了一种超高温真空探针台（UHT-VPS），能够通过单次3w/2w数据测量，在高达前所未有的温度下同时获取材料的电学和热学性质。
...
- **研究目的:** 利用可扩展的、机器学习辅助的材料发现框架，在三元Fe-Co-Zr体系中全面搜索新的硬磁材料。
- **主要发现:** 该研究利用机器学习辅助的材料发现框架，在三元Fe-Co-Zr体系中发现了9种新的热力学稳定化合物和81种有前景的低能亚稳相，并通过化学掺杂优化得到了具有强磁各向异性的$Fe\_5Co\_{18}Zr\_6$和稳定的四元磁性$Fe\_5Co\_{16}Zr\_6Mn\_4$化合物。
...
- **研究目的:** 报告一种用于探测量子相干器件的扫描谐振器的实现，该谐振器无需片上制造即可实现可调谐的相干耦合。
- **主要发现:** 该研究成功实现了一种用于探测量子相干器件的扫描谐振器，该工具能够在毫开尔文温度下以zeptofarad灵敏度和微米空间分辨率进行电容成像，并表征了无片上读出电路的多个Transmon量子比特的能谱和相干时间。
...
- **研究目的:** 利用非磁性$Lu^{3+}$取代$Yb^{3+}$来追踪三角晶格反铁磁体$NaYb\_{1-x}Lu\_xO\_2$中量子无序基态随磁性无序的演变。
- **主要发现:** 该研究通过用非磁性$Lu^{3+}$取代$Yb^{3+}$来追踪三角晶格反铁磁体$NaYb\_{1-x}Lu\_xO\_2$中量子无序基态随磁性无序的演变，其结果为模拟$NaYbO\_2$中量子自旋液体行为出现的理论提供了约束。
...
- **研究目的:** 利用扫描电子显微镜（SEM）中的背散射电子信号分析和菊池衍射图样分析技术，表征不同厚度的$WSe\_2$薄膜样品。
- **主要发现:** 该研究利用扫描电子显微镜（SEM）中的反射菊池衍射（RKD）和多变量统计分析（MSA）技术，成功表征了不同厚度的$WSe\_2$薄膜，实现了二维材料厚度和晶体取向的同时评估。
...
- **研究目的:** 开发一种结合非精确Lanczos方法和树张量网络态（TTNSs）的计算方法，以高效计算量子多体系统的激发本征态。
- **主要发现:** 该研究开发了一种结合非精确Lanczos方法和树张量网络态（TTNSs）的计算方法，成功计算了乙腈、Zundel离子和Eigen离子的激发振动态，并证明了该方法可直接应用于其他量子多体系统。
...
- **研究目的:** 提出一种对有能隙畴壁部分子（partons）的范畴描述，这些部分子与二维拓扑相之间畴壁上的自由度相关联。
- **主要发现:** 该研究提出了一种对有能隙畴壁部分子（partons）的范畴描述，并在此过程中与广义对称性和SymTFT（对称拓扑场论）的概念建立了联系。
...
- **研究目的:** 通过设计一个双层量子霍尔系统，在模糊球面上实现伊辛三临界点，以克服在模糊球面上无法访问自由理论的限制。
- **主要发现:** 该研究通过设计一个双层量子霍尔系统，在模糊球面上实现了伊辛三临界点，并展示了自由玻色子共形标量场论可以从相互作用的电子中涌现，从而扩展了在模糊球面上可实现的共形场论的范围。
