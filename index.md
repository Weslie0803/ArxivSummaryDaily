...
- **研究目的:** 研究手性二维电子气中反演对称Haldane相的配对相互作用。
- **主要发现:** 发现Haldane相的非平凡能带拓扑增强了谷内（${\bf Q} = \pm 2 {\bf K\_D}$）配对磁化率，并预测当声学声子介导配对时，系统支持手性Kekulé超导序。
...
- **研究目的:** 提出一种基于一维任意子Hubbard模型的四冲程量子热机，能够将低温下任意子排斥统计产生的过剩能量转化为有限功。
- **主要发现:** 发现当引入弱相互作用时，功输出在中间统计角达到最大值，表明相互作用和任意子统计非平凡地协同增强了热机性能。
...
- **研究目的:** 研究铁磁CoPt作为加速氧还原反应的候选材料。
- **主要发现:** 通过密度泛函理论计算发现，自旋极化的Co-$d$态由于费米能级处Co-$d$载流子的局部交换劈裂而增强了O$\_{2}$的表面键合。
...
- **研究目的:** 在Mott绝缘态附近的金属相中，通过化学取代和物理压力独立改变无序程度和电子关联强度，以区分它们各自对电荷输运的影响。
- **主要发现:** 观察到在固定无序水平下，残余电阻率$\rho\_0$与$A$系数呈线性增长，并提出这与化学势涨落的方差$\sigma\_\mu^2$有关，表现为$\rho\_0 \propto A\,\sigma\_\mu^2$。
...
- **研究目的:** 引入电子结构因子作为衍射的相敏贡献，直接编码占据能带波函数的性质。
- **主要发现:** 结果表明衍射可以同等地探测电子拓扑和磁有序，为关联电子系统的相敏结构表征开辟了新途径。
...
- **研究目的:** 受光子学中拓扑频率梳实验的启发，理论上在二维三角斯格明子晶格中引入拓扑磁振频率梳（MFCs）。
- **主要发现:** 发现这些拓扑MFCs源于手性边缘模式之间的非线性四磁振散射，由双频驱动激活且无振幅阈值。
...
- **研究目的:** 利用计算模拟，通过xTB软件中实现的半经验紧束缚方法（GFN2-xTB），研究离子化草甘膦与单壁碳纳米管（CNT）之间的相互作用。
- **主要发现:** 研究结果表明，碳纳米管凭借其纳米毛细管性、孔隙率和广阔表面积等特性，在草甘膦污染的环境监测和修复方面具有应用前景。
...
- **研究目的:** 回顾了第二量子化表示的量子化学问题及其在该领域所面临的特殊挑战的最新进展。
- **主要发现:** 展示了这些模型（以及其他源自机器学习的表示）如何作为计算从头算化学性质的新工具，并反过来指导机器学习模型的设计以提取经典数据中的关联模式。
...
- **研究目的:** 展示了一种通过在GaAs三量子点（TQD）中调节点间自旋轨道耦合来实现高保真度自旋控制的方法，其中第三个点提供了对轨道能级的精确控制。
- **主要发现:** 通过随机基准测试，在GaAs量子点器件中实现了99.7%的$\pi$/2门保真度，门时间为4纳秒，即使在该器件中电子自旋相干性通常受到核自旋强超精细相互作用的限制。
...
- **研究目的:** 通过磁控溅射制备CoSn薄膜，并研究了其电阻率随晶体取向的各向异性。
- **主要发现:** CoSn薄膜沿[0001]方向表现出相对较低的电阻率，达到13 micro$\Omega$ cm，并且在[0001]和[2-1-10]（a轴）方向之间存在约十倍的电阻率各向异性。
...
- **研究目的:** 研究了路径纠缠多光子NOON态在平带光子菱形晶格中的传输，并观察到依赖于NOON态相位和光子数的有趣局域化-离域化特征。
- **主要发现:** 在菱形晶格中，对于偶数（奇数）光子数$N$，局域化发生在NOON态的$0 \, (\pi)$相位处，概率为$2^{1-N}$。
...
- **研究目的:** 调查了由传统铌超导体制造的平面约瑟夫森结二极管。
- **主要发现:** 这项工作的关键新颖之处在于展示了光学二极管效应，观察到75 GHz微波辐射的无阈值整流，表明这些二极管表现出接近理想的光学非互易性。
...
- **研究目的:** 结合低能模型和密度泛函理论，证明了具有共面磁序的铁基超导体实现了奇宇称磁态。
- **主要发现:** 我们的发现强调了铁基超导体是探索奇宇称磁性及其与非常规超导共存的平台。
...
- **研究目的:** 报告了单个钯（Pd）纳米晶体中瞬态高应变结构态，这些状态在均匀热膨胀之前通过光学激光电子加热产生。
- **主要发现:** 在足够泵浦-探测延迟下，超过激光能量密度阈值后，布拉格峰分裂成多个峰，表明存在异质应变，随后恢复为单个峰，对应于整个晶格膨胀晶体中的均匀热分布。
...
- **研究目的:** 评估通用机器学习原子间势（uMLIPs）在预测解理能方面的表现，解理能是控制断裂、催化、表面稳定性和界面现象的关键性质。
- **主要发现:** 结果表明训练数据组成比架构复杂性更重要：在强调非平衡构型的OMat24数据集上训练的模型，在未进行显式表面能训练的情况下，平均绝对百分比误差低于6%，并在87%的情况下正确识别了热力学最稳定的表面终止。
...
- **研究目的:** 在具有幂律驱动交流电导率的多晶双钙钛矿A$\_{2}$SmTaO$\_{6}$（A = Ba, Ca）中，研究载流子的跳跃和弛豫如何受晶粒和晶界的影响。
- **主要发现:** 交流电导率和阻抗响应的标度揭示了传导和弛豫时间尺度之间的关联，并表明“受挫”偶极子局部能量景观的不均匀性限制了传导机制在体相尺度上的“普适性”。
...
- **研究目的:** 演示了夹在金属电极之间的铁电薄膜的开关可以通过激光辅助加热进行控制，类似于热辅助磁记录。
- **主要发现:** 同步纳秒激光脉冲对金属顶电极的瞬态加热诱导了平均极化的反转，其变化量$\Delta P^\mathrm{L}$大于剩余极化$P\_\mathrm{r}$。
...
- **研究目的:** 研究了单层到三层CrSBr的光致发光（PL）和PL激发，发现其表现出独特的双重性，支持Frenkel型和Wannier-Mott型激子。
- **主要发现:** 磁光实验揭示了单层和三层系统具有相似的激子响应，而双层薄片则表现出完全不同的特征，这表明能带结构中低能激子种类（A、A'和B）的起源不同。
...
- **研究目的:** 介绍了结合原子力显微镜（AFM）和纳米SQUID传感的轻敲模式SQUID-on-tip显微镜。
- **主要发现:** 通过在没有外部辐射的情况下捕获局部磁、热和电子响应，该技术为研究奇异材料和精密量子电路中的动态现象提供了一种强大的非侵入性途径。

---

### [Remote spin control in Haldane spin chains](http://arxiv.org/abs/2508.21544v1)
- **作者:** Y. del Castillo, A. Ferrón, J. Fernández-Rossier
- **研究目的:** 考虑利用另一端边缘的弱局部扰动，对Haldane自旋链边缘分数自旋的量子态进行远程操控。
- **主要发现:** 利用Landau-Zener协议，展示了通过对链一端局部磁场的控制，可以绝热地切换链另一端的磁化方向。
...
- **研究目的:** 在GaN(0001)衬底上外延生长了2H-TaS$\_2$少层薄膜。
- **主要发现:** 原位X射线光电子能谱显示核心能级位移与电子从n-GaN(0001)转移到2H-TaS$\_2$外延层以及界面附近高浓度氮空位的形成一致。
...
- **研究目的:** 训练一个卷积神经网络，用于根据输入图像对低分辨率（即大块尺寸）基础模型特征进行上采样。
- **主要发现:** 结果表明，使用这些深度特征进行交互式分割，比训练或微调传统卷积网络能更快、用更少的标签生成高质量的分割。
...
- **研究目的:** 全面调查了由单层MoS$\_2$与有机分子苝（P）、苝二酰亚胺（PDI）和苝橙（PO）耦合形成的杂化界面。
- **主要发现:** 杨氏模量和泊松比在所有情况下都表现出显著的各向异性，其中PO由于空间效应和永久偶极子而表现出最大的各向异性。
...
- **研究目的:** 通过重复频率为1兆赫兹的双周期光学脉冲，调制了ITO纳米晶体在其零介电常数波长附近的透射率。
- **主要发现:** 灵敏的场谱测量首次解析了ITO的瞬态电场响应，表明在高达1.2 mJ/cm2的激发能量密度下，调制保持可逆，而在超过3.3 mJ/cm2时则完全不可逆。
...
- **研究目的:** 利用霍尔效应证明了LaCrGe3中两种特殊铁磁相的存在。
- **主要发现:** 在固定磁场下连续的温度依赖性霍尔电阻率测量清楚地证明了这些相的存在，且与外加磁场方向无关。
...
- **研究目的:** 探索通过调整衬底-金属相互作用来控制沉积面心立方 (fcc) 金属薄膜生长形态的策略。
- **主要发现:** 模拟结果表明，调节衬底相互作用强度可以有效促进岛状或逐层生长模式，并发现Au、Pd、Pt对衬底相互作用强度变化最敏感，且热真空退火能进一步改善互连相关指标。
...
- **研究目的:** 通过引入强各向异性捕获势并追踪紧密限制方向，从二维Chern-Simons-Schrödinger系统推导出一维有效方程。
- **主要发现:** 结果表明，波导松散方向的有效动力学由经典一维五次非线性薛定谔 (NLS) 方程控制。
...
- **研究目的:** 全面综述二维Janus材料的理论预测、实验制备、调制策略、可修改特性及其在能源和信息领域的应用。
- **主要发现:** 二维Janus材料因其独特的破缺空间对称性和新兴特性，在光学、催化、压电、电化学、热电、磁学和电子学等领域为解决能源和信息问题提供了有前景的解决方案。
...
- **研究目的:** 引入一种簇版本无限时演化块截断 (cluster iTEBD) 算法，以提高强关联物理中一维量子多体系统基态计算的准确性。
- **主要发现:** 该方法通过重新定义波函数 ansatz 并结合优化截断方案，在保持大致相同计算复杂度的前提下，显著提高了无能隙自旋-$1/2$ 海森堡链、自旋-1各向异性XXZD链和自旋-$1/2$ 扭曲三角棱柱等非平凡案例中物理量的准确性。
...
- **研究目的:** 在动力学平均场理论框架内，精确评估 Falicov-Kimball 模型中局域 $f$ 粒子态的非时序关联函数 (OTOC)。
- **主要发现:** 通过计算中使用的块Toeplitz矩阵的缠绕数区分了量子混沌和量子加扰的不同机制，并讨论了这些费米子OTOC及其Lyapunov指数与无序量子自旋模型OTOC的相似性。
...
- **研究目的:** 首次展示在铁电钽酸铌钾衬底上沉积非晶态LaAlO$\_3$薄膜，以实现可调谐二维电子气 ($2\text{DEG}$) 系统。
- **主要发现:** 在高温高氧压下生长的界面表现出良好的金属导电性，并在低于 $108 \text{ K}$ 时观察到具有明显滞后和持续电场调制电阻的 $2\text{DEG}$，电阻调制率在 $7 \text{ K}$ 时达到 $11.6\%$。
...
- **研究目的:** 理论研究基于单层 $1\text{T'}$-WTe$\_2$ 的约瑟夫森结中的约瑟夫森二极管效应 (JDE)，并探索其边缘依赖性。
- **主要发现:** 发现WTe$\_2$纳米带的不同边缘终止可以产生非对称边缘态，在时间反演对称性被磁通量破坏时，纯粹由这些非对称边缘引起的JDE显著增强，且通过调节化学势进入体带，JDE效率可提高超过 $50\%$。
...
- **研究目的:** 评估密度泛函理论 (DFT) 计算在石墨烯负载单Pt原子上小气体分子（O$\_2$, CO, CO$\_2$, O）吸附性质方面的性能，并与扩散蒙特卡罗 (DMC) 计算进行基准测试。
- **主要发现:** DMC计算揭示了与DFT结果显著不同的吸附能景观，特别是O$\_2$的最低能量构型和自旋态存在差异，强调了在催化研究中需要更复杂的计算方法来提高反应机制预测的准确性。
...
- **研究目的:** 理论研究电子-电子相互作用在一维磁化螺旋边缘态与s波超导体耦合系统中的作用。
- **主要发现:** 利用玻色化和重整化群技术，发现超导配对促进自旋密度波关联，而单重态和三重态配对关联受到抑制，尤其是在吸引相互作用下；在超螺旋态下，微扰塞曼场在排斥相互作用下增强了自旋单重态和自旋三重态-x配对。
...
- **研究目的:** 开发一种混合量子-经典框架，结合多组态自洽场 (MCSCF) 和变分量子本征求解器 (VQE)，以解决传统密度泛函理论 (DFT) 在强关联系统模拟中的局限性。
- **主要发现:** 该框架在水解离基准测试中表现出色，并能为石墨烯类似物与水以及过渡金属（Fe、Co、Ni）在石墨烯类似物上的吸附提供与高精度量子方法一致的结合能，揭示了DFT常误报的强电荷转移和多参考特性现象。
...
- **研究目的:** 探索通过原子级尺寸选择的团簇束源制备面心立方 (FCC) W-Cu纳米合金，并研究其原子级精确的混溶性演变及其不对称性。
- **主要发现:** 首次获得了W-Cu纳米合金的不对称混溶纳米相图，其中富W组分有利于混合，临界尺寸约为6000个原子，远超经典理论预测，并通过密度泛函理论 (DFT) 计算揭示了互应变诱导机制，该机制同时降低了表面能并实现了原子级尺寸。
...
- **研究目的:** 利用角分辨光电子能谱 (ARPES) 实验验证层状电子化合物 YCl 中费米能级处骰子晶格平带的存在。
- **主要发现:** ARPES测量明确识别出YCl中的两组骰子晶格能带，包括费米能级附近几乎无色散的平带，这与第一性原理计算结果高度吻合，证实了YCl作为骰子金属原型的地位，并揭示了阴离子电子晶格作为实现稀有晶格几何和电子结构的新方案。
...
- **研究目的:** 利用光学超快时间分辨泵浦-探测光谱，全面研究重掺杂单层铜酸盐 (Bi,Pb)$\_{2}$Sr$\_{2}$CuO$\_{6+\delta}$ (Pb-Bi2201) 在赝能隙和超导相终点附近的瞬态光致反射率动力学。
- **主要发现:** 在Pb-Bi2201中，靠近赝能隙终点时，高于 $T\_{\text{c}}$ 的瞬态反射率动力学表现出与优化掺杂La-Bi2201相似的赝能隙响应，且在低温下弛豫时间呈现幂律发散 $\tau \sim 10\hbar/k\_{\text{B}}T$，表明在赝能隙终点掺杂处存在量子临界行为。
...
- **研究目的:** 探索电子关联强度如何有效控制Weyl铁磁体Co$\_3$Sn$\_2$S$\_2$中费米弧 (FA) 的拓扑结构和连通性。
- **主要发现:** 结合密度泛函理论和动力学平均场理论 (DFT+DMFT) 计算表明，电子关联重整化了费米能级附近的能带并改变了Weyl点的能量和位置，从而导致FA的拓扑结构发生显著变化，尤其是在Co终止表面，FA连接了相邻布里渊区中的Weyl点，这与DFT计算结果形成鲜明对比。
...
- **研究目的:** 提出一种基于赝势平面波的刚性Muffin-Tin近似 (RMTA) 实现，为声子介导超导体的快速建模提供计算效率更高的替代方案。
- **主要发现:** 该方法能够评估元素过渡金属及其化合物的角动量分辨电子-声子矩阵元和McMillan-Hopfield参数，与全势线性化增强平面波计算结果高度一致，显著降低了筛选候选超导体的计算成本。
...
- **研究目的:** 探索使用超导电子学 (SCE) 加速全同态加密 (FHE) 中的数论变换 (NTT)，以实现超越传统CMOS的更高性能和能效。
- **主要发现:** 提出的SCE-NTT硬件加速器实现了 $N=128$ 32位系数的NTT-128单元，以 $34 \text{ GHz}$ 达到 $531$ 百万 NTT/秒，比最先进的CMOS等效产品快 $100$ 倍以上，并展示了SCE-based加速器在后量子时代可扩展、节能安全计算方面的巨大潜力。
...
- **研究目的:** 提出一个超越传统耦合振子模型的理论框架，以全面解释激子-极化子相互作用的集体和相干性质如何影响其辐射衰变。
- **主要发现:** 证明了强激子-光子相互作用可以通过破坏性干涉强烈抑制极化子辐射，从而产生具有无限长辐射寿命的极化子连续谱束缚态，为设计具有定制辐射特性的长寿命激子-极化子提供了新原理。
...
- **研究目的:** 通过系统比较由纯Co、Ni和Fe组成的参考层，研究单飞秒激光脉冲在铁磁自旋阀中诱导的磁化反转过程。
- **主要发现:** 发现反平行到平行切换在所有三种元素中均可观察到，但平行到反平行切换仅在Co参考层中发生，这归因于参考材料独特的超快磁化动力学，支持了平行到反平行切换需要参考层快速再磁化以产生显著负自旋电流的假设。
...
- **研究目的:** 证明在广泛的各向同性自旋系统中，在非常普遍的条件下会产生四次势，并研究其非谐集体振荡及其光谱特征。
- **主要发现:** 数值模拟表明，四次振幅自旋振荡会产生一个随温度以特征幂律增长的涨落诱导自旋波能隙，为描述软自旋模式提供了一个通用理论框架，其预测的温度依赖能隙可在非弹性中子散射实验中进行验证。
...
- **研究目的:** 开发一种名为LREI（低秩本征模式积分）的内存和时间高效方案，用于求解控制开放量子系统自旋动力学的量子Landau-Lifshitz (q-LL) 和量子Landau-Lifshitz-Gilbert (q-LLG) 方程。
- **主要发现:** LREI通过利用密度矩阵的低秩结构和哈密顿量的稀疏性，将Runge-Kutta和Adams-Bashforth方案的每步计算复杂度从 $\mathcal{O}(N^3)$ 降低到 $\mathcal{O}(r^2N)$，内存成本从 $\mathcal{O}(N^2)$ 降低到 $\mathcal{O}(rN)$，使得在标准笔记本电脑上模拟多达20个自旋的系统成为可能。
...
- **主要发现:** 预测结果通过有限元分析得到验证，证实无序和随机性会影响晶格损伤起始载荷的平均值和方差，平均失效载荷通常降低，方差随无序和随机性水平的提高而增加，并预测了断裂内聚长度和代表体积单元尺寸。
...
- **研究目的:** 通过激光烧蚀铝、镍、钠和因瓦合金，在高达 $475 \text{ GPa}$ 的压力下，利用X射线衍射应变测量这些材料在纵波中的压缩轴向应变。
- **主要发现:** 发展了一种基于压缩波中主晶体学应变的布拉格衍射X射线三维通用解，并利用原位测量量化了铝、镍、钠和因瓦合金的压缩轴向比，分别为 $0.65, 1.05, 0.88, 1.58$，在 $291, 402, 409, 367 \text{ GPa}$ 的压力下。
...
- **研究目的:** 利用密度泛函理论 (DFT) 模拟两种工业上重要的有机金属体系 [M(COD)Cl]$\_2$ (M=Ir/Rh, COD=1,5-环辛二烯) 在X射线辐射下的电子结构，以揭示分子间损伤过程。
- **主要发现:** 该方法允许将计算光谱直接与实验数据（X射线光电子价带光谱）进行比较，从而实现了单个原子态与电子结构的有效关联，展示了DFT在研究辐射损伤复杂系统中的巨大潜力。
...
- **研究目的:** 利用X射线织构和衍射断层扫描技术，揭示二氧化硅-白铅矿生物形态在三维空间中的局部晶体织构，以深入理解其形成过程和合成参数对纳米级特性和微观形状的影响。
- **主要发现:** 发现不同形态的生物形态具有显著不同的生长模式，并且单个结构内部的晶体特性也存在显著差异，从而区分了不同的生长机制，并推断出共沉淀二氧化硅如何塑造晶体颗粒。
...
- **研究目的:** 探索软微凝胶网络如何稳定和延长无喷嘴水射流，以克服毛细管驱动破裂对高速液体射流稳定性的限制。
- **主要发现:** 发现软的、低交联剂密度的PNIPAM微凝胶在空气-水界面形成弹性界面网络，抑制表面张力恢复，延迟Rayleigh-Plateau不稳定性，并将表面声波驱动的射流长度延长高达 $44\%$，为无针药物输送和高速打印等应用奠定了基础。
...
-   **研究目的:** 调查有序量子磁体中的自旋动力学，特别是在相互作用主导的非微扰区域，以解释异常自旋激发连续谱。
-   **主要发现:** 通过截断希尔伯特空间精确对角化 (THED) 和矩阵乘积态 (MPS) 模拟，确认了三角晶格反铁磁体 $1/3$-平台相中存在双磁子束缚态和共振，并成功解释了实验中观察到的异常散射连续谱。
...
-   **研究目的:** 通过研究Kitaev蜂窝自旋液体中Anyon准粒子的色散，理解其在$\Gamma$和$\Gamma'$耦合扰动下向竞争相的相变。
-   **主要发现:** Anyon能隙的闭合可以解释向锯齿形、条纹状、$120^\circ$和非公度螺旋相的相变，并且当反铁磁Kitaev模型受到铁磁$\Gamma$相互作用扰动时，会产生自发对称破缺和Kitaev自旋液体分数化模式共存的物态。
...
-   **研究目的:** 引入一类具有反演对称性的单味平带模型，以解释菱形石墨烯中观察到的超导现象，并研究其配对机制。
-   **主要发现:** 构建了单味平带模型，其中具有相反宇称的轨道之间的局部吸引相互作用导致精确的超导基态，其配对性质由平带量子几何和相互作用决定，并且通常是拓扑的。
...
-   **研究目的:** 提出一个通用机制来解释二维自旋轨道耦合六方体系中，位移场 $D$ 如何驱动超导电性与谷铁磁性之间的转变。
-   **主要发现:** 在范霍夫奇点位于费米能级时，弱位移场 $D < D\_c$ 下手性 $d/p$-波超导占主导，而强位移场 $D > D\_c$ 下则出现谷铁磁相，该相因谷调制磁化而具有空间非均匀性。
...
-   **研究目的:** 在双轨道自旋费米子格点系统中构建弱遍历性破缺的子空间，并研究其中非传统超导配对关联的性质。
-   **主要发现:** 构建了动态解耦的子空间，其中包含最强的轨道间、自旋单线态或自旋三线态长程超导配对关联，这些子空间由结合了 $2e$ 和 $4e$ 配对/簇贡献的群不变量子多体疤痕族构成。

---

### [Charge density wave induced gapped nodal line](http://arxiv.org/abs/2508.21117v1)
-   **作者:** Sergey Alekseev, Lei Chen, Jennifer Cano
-   **研究目的:** 调查电荷密度波 (CDW) 序与拓扑节线态在方格材料中的相互作用，并解释其对节线能隙的影响。
-   **主要发现:** Ginzburg-Landau理论预测CDW不稳定性通常会在费米能级打开一个能隙，同时保留节线交叉，但当费米能级接近节线时，节线处的态密度会降低，并且当CDW矢量 $\mathbf{Q}$ 趋于零时，能隙会打开，因为滑移对称性被显式破缺。
